...
function sanitizeFilename($filename) {
	// Remove accents and convert to ASCII-safe characters (optional but recommended)
	$filename = iconv('UTF-8', 'ASCII//TRANSLIT', $filename);
	// Remove any characters that are not alphanumeric, hyphen, underscore, or period
	$filename = preg_replace('/[^a-zA-Z0-9\-\._]/', '', $filename);
	// Replace multiple periods with a single period (e.g., "file..name.txt" becomes "file.name.txt")
	$filename = preg_replace('/\.(?=\.)/', '', $filename);
	// Ensure only one period precedes the extension (e.g., "file.name.with.dots.txt" becomes "file_name_with_dots.txt")
	$parts = explode('.', $filename);
	$extension = array_pop($parts);
	$filename = implode('_', $parts) . '.' . $extension;
	// Trim leading/trailing hyphens or underscores
	$filename = trim($filename, '-_');
	// Remove leadig . - no hidden files
...
class Up_Load
{
	...
	private function vetUpload ($fec=true)
	{
		...
		if (function_exists('sanitizeFilename')) {
			...
			if (!$file['name']) throw new Exception('File name is not valid');
		}
