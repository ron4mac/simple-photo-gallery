...
class ImageProcessor extends ImageProc
{
	...
	public function resizeToContainer ($w, $h, $dest, $mth='fit')
	{
		...
		case 'fit':
			...
			$cmd = "convert {$_s} -resize \"{$nw}x{$nh}>\" -quality 90 {$_d} 2>&1";
			break;
		case 'fill':
			...
			$sw = (int)($w*$wr);
			$sh = (int)($h*$hr);
			$cmd = "convert {$_s} -resize \"{$sw}x{$sh}^\" -gravity center -extent {$w}x{$h} -quality 90 {$_d} 2>&1";
			break;
		}
	...
	public function createThumb ($dest, $ext, $maxW=120, $maxH=120, $sqr=true)
	{
		...
		$cmd = "convert {$_s} -thumbnail {$sw}x{$sh}^ -gravity center -extent {$maxW}x{$maxH} -sharpen 0x1 -quality 90 {$_d}  2>&1";
		exec($cmd, $output, $retval);
