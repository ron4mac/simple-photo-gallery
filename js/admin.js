...
function newFreq (evt,elm) {
	...
	if (fnam) {
		...
		alert('Please enter an appropriate folder name');
		return false;
	}
	return true;
}
...
function doCfg (L=1) {
	...
	.then(data => {
		//console.log(data);
		document.getElementById('cfgElms').innerHTML = data;
		dlg.showModal();
	})
	.catch(err => alert('Failure: '+err));
}
...
function foldset (foldp) {
	let dlg = document.getElementById('foldDlg');
	let parms = new URLSearchParams('fold=1&fld='+foldp);
	fetch('admin.php', {method:'POST',body:parms})
	.then(resp => { if (!resp.ok) throw new Error(`HTTP ${resp.status}`); if (false && json) return resp.json(); else return resp.text() })
	.then(data => {
		//console.log(data);
		document.getElementById('foldElms').innerHTML = data;
		dlg.showModal();
	})
	.catch(err => alert('Failure: '+err));
}
...
<div>Files queued: <span id="qcount">0</span>
	<div class="acti" id="qstop"><i class="fa fa-pause-circle pausQ" title="stop queue" onclick="H5uQctrl.stop()"></i></div>
	<div class="acti" id="qgocan"><i class="fa fa-play-circle playQ" title="resume queue" onclick="H5uQctrl.go()"></i><i class="fa fa-times cancelQ" title="cancel queue" alt="" onclick="H5uQctrl.cancel()"></i></div>
</div>
